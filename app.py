...
cliente = "EXBR"
st.markdown(
    f'<div class="titulo-pagina">Dashboard de Recebimentos - {cliente}</div>',
    unsafe_allow_html=True
)
st.markdown(
    '<div class="subtitulo-pagina">Visão consolidada de ONLINE e HTM</div>',
    unsafe_allow_html=True
)
...
# Diagnóstico
st.markdown('<div class="secao-titulo">Diagnóstico</div>', unsafe_allow_html=True)

if inadimplencia > 20:
    st.error("Inadimplência alta. Necessária ação imediata de cobrança.")
elif inadimplencia > 10:
    st.warning("Inadimplência moderada. Recomenda-se atuar primeiro nos maiores devedores.")
else:
    st.success("Inadimplência sob controle.")
